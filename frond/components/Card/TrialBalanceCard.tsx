...
import SubTrialBalanceCard from "./SubTrialBalanceCard";
import { useState } from "react";
import TotalTrialBalanceTbodyTr from "../Tables/Tbody/TotalTrialBalanceTbodyTr";

interface TrialBalanceCardProps {}

const TrialBalanceCard: React.FC<TrialBalanceCardProps> = ({}) => {
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          {data?.getaccount.map((ac) => (
            <tbody className="bg-white" key={ac.account_id}>
              ...
              <SubTrialBalanceCard Id={ac.account_id} from={From} to={To} />
            </tbody>
          ))}
          <TotalTrialBalanceTbodyTr from={From} to={To} />
        </table>
      </div>
    </div>
  );
